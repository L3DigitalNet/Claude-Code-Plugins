...
**`config portfolio-write`** affects behavior across **all repos in the portfolio** — staleness thresholds, module toggles, expertise level, and per-repo overrides all inherit from portfolio defaults. Always run `config portfolio-read` first and preview with `--dry-run` before writing.
