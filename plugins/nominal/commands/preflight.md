...
**If it DOES exist:** Read the profile. Check `_schema_version` against the current plugin version (read from `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json`). If they differ, apply migrations as described in the environment-profile reference.
