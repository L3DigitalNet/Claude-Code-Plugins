...
**NOTE:** Per-plugin loops (Phases 1–4 below) do NOT run their own stash/pop. The global stash above covers the entire batch. Each plugin release only stages its own files, so the working tree stays clean between plugins.
