#!/usr/bin/env bash
# tests/run-bats.sh — Run the bats suite for opus-context.
# Bypasses the bats v1.13.0 wrapper bug on Fedora 44 / bash 5.3.9 that
# silently drops all test output. See plugins/release-pipeline/tests/run-bats.sh
