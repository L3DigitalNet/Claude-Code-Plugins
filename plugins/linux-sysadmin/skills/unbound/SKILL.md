...
description: >
  ...
  Triggers on: unbound, recursive resolver, DNSSEC, DNS resolver, root hints,
  unbound.conf, unbound-control.
globs:
  - "**/unbound.conf"
  - "**/unbound/**/*.conf"
...
- **Cache size tuning**: Default cache is small. Increase `msg-cache-size` and `rrset-cache-size` (set rrset to 2× msg-cache) for better hit rates on home networks.
