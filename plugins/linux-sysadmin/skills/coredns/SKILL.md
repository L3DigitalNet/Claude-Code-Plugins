...
coredns -conf /etc/coredns/Corefile -validate   # check config syntax
dig @127.0.0.1 example.com A                    # test DNS query
```
...
## See Also
- **bind9** — full-featured authoritative DNS server and recursive resolver with DNSSEC
- **unbound** — recursive-only DNS resolver optimized for caching and DNSSEC validation
- **dnsmasq** — lightweight DNS forwarder and DHCP combo for small/home networks
