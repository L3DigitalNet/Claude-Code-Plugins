...
# Test round-trip publish/subscribe
mosquitto_sub -h localhost -t 'test/topic' -C 1 &
sleep 0.2 && mosquitto_pub -h localhost -t 'test/topic' -m 'hello'
...
| Test with authentication | `mosquitto_pub -h localhost -t test/topic -m "hello" -u myuser -P "$MQTT_PASS"` |
...
| Create credential file | `mosquitto_passwd -c /etc/mosquitto/passwd myuser` (add user: `-b` flag) |
| Add user to credential file | `mosquitto_passwd -b /etc/mosquitto/passwd anotheruser "$MQTT_PASS"` |
...
- **ACL file changes require reload, not restart**: `systemctl reload mosquitto` re-reads the ACL and credential files without dropping existing connections.

## See Also

- **zigbee2mqtt** — Zigbee device bridge that publishes to Mosquitto for smart home automation
- **zwave-js** — Z-Wave device controller that uses MQTT for device state and commands
- **node-red** — flow-based automation tool that consumes and produces MQTT messages
- **gotify** — self-hosted push notifications; alternative delivery channel alongside MQTT
