...
export function createServer() {
    ...
    const server = new Server({ name: 'plugin-test-harness', version: '1.0.0' }, { capabilities: { tools: {} } });
    // Dynamic tool list
    ...
    async function dispatch(toolName, args) {
        ...
        try {
            ...
            switch (toolName) {
                case 'pth_preflight': {
                    const result = await sessionManager.preflight(args);
                    return respond(result);
                }
                case 'pth_start_session': {
                    const result = await sessionManager.startSession(args);
                ...
                case 'pth_resume_session': {
                    const result = await sessionManager.resumeSession(args);
                    currentSession = result.state;
                ...
                case 'pth_end_session': {
                    if (!currentSession)
                        return respond('No active session.');
    ...
    async function handleSessionTool(toolName, args, session) {
        const respond = (text) => ({ content: [{ type: 'text', text }] });
        ...
        switch (toolName) {
            ...
            case 'pth_list_tests': {
                ...
                if (tests.length === 0)
                    return respond('No tests match the filter.');
            ...
            case 'pth_record_result': {
                ...
                if (!test)
                    return respond(`Unknown test id: ${testId}`);
            ...
            case 'pth_get_results': {
                ...
                if (all.length === 0)
                    return respond('No tests in suite. Run pth_generate_tests first.');
            ...
            case 'pth_get_test_impact': {
                const { files } = args;
                const impacted = store.filter(t => files.some(f => {
            ...
            // ── Fixes ──────────────────────────────────────────────────────
            case 'pth_apply_fix': {
                const { files, commitTitle, testId, category } = args;
                const trailers = {
            ...
            case 'pth_sync_to_cache': {
                ...
                try {
                    ...
                    return respond(`Cache sync failed: ${e.message}\nCache path: ${cachePath}`);
                }
            }
            ...
            case 'pth_get_fix_history': {
                ...
                if (history.length === 0)
                    return respond('No fix commits on this session branch yet.');
                const lines = history.map(fix => `${fix.commitHash.slice(0, 7)} ${fix.commitTitle}` +
            ...
            case 'pth_diff_session': {
                ...
                if (!diff.trim())
                    return respond('No changes on session branch yet.');
            ...
            case 'pth_get_iteration_status': {
                ...
                const history = snapshots.map((s, i) => `| ${i + 1} | ${s.passing} | ${s.failing} | ${s.fixesApplied} |`).join('\n');
                return respond([
