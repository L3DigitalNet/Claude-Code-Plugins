...
async function main() {
	...
	try {
		...
		const hasComponents =
			Array.isArray(apiConfig.components) && apiConfig.components.length > 0
		...
		record(
			'HA Connection: components',
			hasComponents,
			`${apiConfig.components.length} components`
		)
	...
	try {
		...
		record(
			'Entity Query: light.ceiling_lights exists',
			entityIds.has('light.ceiling_lights')
		)
		record(
			'Entity Query: sensor.outside_temperature exists',
			entityIds.has('sensor.outside_temperature')
		)
	} catch (e) {
	...
	try {
		...
		record(
			'Service Discovery: services returned',
			services.length > 0,
			`${services.length} domains`
		)
	...
	try {
		...
		const serviceExists =
			lightServices && Object.keys(lightServices.services).includes('turn_on')
		...
		record(
			'Dry Run: non-existent entity rejected',
			!fakeEntity,
			'light.nonexistent not found'
		)
	} catch (e) {
	...
	try {
		...
		const states = await haApi('/api/states', 'GET', null, token)
		const demoEntities = states.filter(
			(s) => s.attributes.friendly_name && !s.entity_id.startsWith('person.')
		)
		record(
			'Device Registry: demo entities have friendly names',
			demoEntities.length > 5,
...
`
	...
	await import('fs').then((fs) =>
		fs.writeFileSync(
			join(homedir(), 'ha-plugin-test-workspace', 'MCP_TEST_RESULTS.md'),
			resultsMd
		)
	)
...
const MCP_DIST =
	'/home/chris/projects/Claude-Code-Plugins/plugins/home-assistant-dev/mcp-server/dist'
...
async function main() {
	...
	try {
		...
		record(
			'WS Connect',
			info.connected,
			`HA v${info.version}, ${info.components.length} components`
		)
	...
	try {
		const valid = await client.validateServiceCall(
			'light',
			'turn_on',
			{},
		...
		const invalid = await client.validateServiceCall(
			'light',
			'turn_on',
			{},
		...
		record(
			'validateServiceCall: invalid entity rejected',
			!invalid.valid,
			invalid.errors.join(', ')
		)
		...
		record(
			'validateServiceCall: fake service rejected',
			!noService.valid,
	...
	try {
		...
		const redacted = safety.redactSensitiveData({
			api_key: 'secret123',
			brightness: 255,
		})
		record(
			'Safety: redacts sensitive data',
