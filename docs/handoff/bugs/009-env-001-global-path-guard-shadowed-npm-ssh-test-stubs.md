...
Bug 8's fix (`4f9fd1c`, `19595e2`) applied ENV-001's guard uniformly: `export PATH="/usr/bin:/bin:$PATH"` at the top of every python3-invoking release-pipeline and up-docs script. That's safe when python3 is the *only* external command the script calls afterward — but two scripts also call a **second** externally-stubbable command later in the same file:
