...
| repo-hygiene | 1.4.3 | **Marketplace-live at 1.4.3** (marketplace `source` serves `./plugins` from `main`) but **NOT yet GitHub-released** — latest tag is `repo-hygiene/v1.4.1`. Untagged delta: 1.4.2 (commit `e3e0b11`, hygiene-semantic-auditor gains handoff-v3 awareness — never flag canonical `docs/handoff/` files, stray `docs/handoff.md` is info-not-warn, defer conformance to `validate-layout.sh`) + 1.4.3 (commit `3ca0d10`, de-dup semantic auditor vs Step 1 scripts + doc-drift fix). Released baseline 1.4.1 (2026-05-25): TEST-003 fix, 40/40 tests. **Pending:** `/release-pipeline:release` to tag v1.4.3. |
| up-docs | 0.9.1 | Released 2026-05-30 (tag `up-docs/v0.9.1`, GitHub release published); reliable retired-V1/V2-label scan + drift-coverage docs (commit `4e52ba9`). Prior 0.9.0 (2026-05-30): handoff v3 alignment (12 divergences: 2🔴/6🟡/4🟢) — propagate-repo emits v3 three-line AGENTS.md block + bug-body `## Lesson`, enforces CLAUDE.md ≤2048 / AGENTS.md ≤4096 caps, audits `docs/handoff/specs-plans.md`, route-first `state.md` trim, bug-index verify; drift auditor runs `validate-layout.sh` conformance phase. 51 bats + 29 pytest. |
...
- **Two marketplace-live manifest bumps await a GitHub release tag:** repo-hygiene v1.4.3 and test-driver v0.6.2 (both bumped on `main`, never tagged). Run `/release-pipeline:release` to reconcile. Installers already receive these versions; only the GitHub release/tag artifact lags.
