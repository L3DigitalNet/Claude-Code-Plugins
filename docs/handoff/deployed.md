...
| test-driver | 0.6.2 | Released 2026-06-08 (tag `test-driver/v0.6.2`, GitHub release published). 0.6.2 (commit `e3e0b11`) drops references to the deleted opus-context plugin while preserving the full-context reading philosophy verbatim. Prior 0.6.1 (2026-05-25): TEST-003 fix, 57/57 tests. |
...
- **release-pipeline PATH-shim hardening committed but unreleased** (`4f9fd1c`, Bug 8): 7 scripts guard against uv-strict-python's python3 shims. Ships with the next release-pipeline release (5 commits pending as of 2026-06-12).
