...
- `specpipe` is **stdlib-only** (argparse, re, json, pathlib, shlex, subprocess) and deliberately has **no Python project machinery** — no `pyproject.toml`, no venv, no lockfile. It is a plain package directory imported via `PYTHONPATH`, because `uv run` against a project would lock/sync and write `.venv/`+`uv.lock` into the plugin root — which dirties the source repo and is brittle in the installed-plugin cache (`${CLAUDE_PLUGIN_ROOT}` must not receive persistent state). Canonical invocation: `PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/scripts/specpipe" uv run --no-project python -B -m specpipe …` (uv supplies a suitable interpreter without touching the plugin tree; runtime floor Python ≥ 3.11). Acceptance includes a clean-tree proof that covers **ignored** artifacts too (plain `git status` would stay silent on gitignored `.venv/`): after any specpipe invocation, `git status --short --ignored plugins/spec-pipeline` is empty and no `.venv/`, `uv.lock`, `.pytest_cache/`, `.ruff_cache/`, or `__pycache__/` exists under `plugins/spec-pipeline`.
...
- Query subcommands (`validate phase-plan|spec|plan`, `next-phase`, `status`) support `--json` (machine-readable findings/results) alongside human-readable output. Mutating/state subcommands (`set-status`, `record-red`, `record-green`, `rounds`, `init-project`) communicate via exit codes and stable single-line output — no `--json`.
...
| `record-red --cmd <test-cmd> --task <id> --audit <path> [--framework pytest\|generic] [--expect-failure-regex <re>] [--timeout N]` | Runs the command under the execution-safety contract (below). RED requires a **positive failure signature**, not merely a non-zero exit: for the default `pytest` framework the output must contain a genuine test-failure marker (`N failed` / `FAILED` summary lines) AND must not match collection/import/syntax-error signatures — so "no tests ran", usage/config errors, runner crashes, and arbitrary `exit 1` commands are all REJECTED. `--framework generic` (bats/Jest/other runners) preserves the fails-for-the-right-reason rule via `--expect-failure-regex`, which is **mandatory with generic**: the output must match it (e.g. the expected failing assertion or "not defined" message) or RED is REJECTED; `generic` without a regex is a bad invocation (exit 2) — there is no verification-free RED path. Appends an evidence block (task, redacted command, timestamp, failure excerpt) to the audit file. Collection error, unmatched expectation, unexpected pass, or timeout = exit 1. |
...
| `rounds <state-file> --gate spec\|plan\|final [--increment] [--reset]` | Round counters with caps 3/3/5. `--increment` past the cap = exit 1 (the skill's signal to stop looping and record open findings). Omitting `--increment` is the read-only check (prints used/cap; exit 1 if over cap). `--reset` zeroes all gates at phase start. |
...
9. specpipe invocations leave the plugin tree clean **including ignored artifacts**: after running the canonical invocation, `git status --short --ignored plugins/spec-pipeline` is empty and `find plugins/spec-pipeline \( -name .venv -o -name uv.lock -o -name .pytest_cache -o -name .ruff_cache -o -name __pycache__ \)` finds nothing (gitignore would otherwise hide exactly the forbidden state).
...
| 4 (2026-07-01, `docs/codex-reviews/2026-07-01-185410-codex-spec-review-round4.md`) | Needs minor correction (0 blocking / 1 medium; SA-001..006 + SA-NEW-001 all resolved) | SA-NEW-002 (clean-tree proof blind to gitignored artifacts) → check upgraded to `git status --short --ignored` plus explicit `find` absence checks for `.venv`/`uv.lock`/cache dirs. Converged per loop rules (minor-only). |
| plan-r1 drift (2026-07-01, `docs/codex-reviews/2026-07-01-190012-codex-plan-review-round1.md`) | Spec contracts tightened by plan-review findings | CR-001 → RED requires a positive pytest failure signature (`N failed`/`FAILED`), not merely non-zero exit. CR-003 → `--json` narrowed to query subcommands; `rounds` check = omitting `--increment` (no separate `--check` flag). |
