...
- [ ] **spec-pipeline follow-ups** — live smoke test in a fresh session (needs plugin install + cache sync), `/release-pipeline:release` 0.1.0, decide deprecation of `author-master-spec` + `autonomous-phase-execution` in `agent-configs`. Remaining 0.1.x hygiene backlog (word-boundary scans done in `ec74a16`): `~~~` fence support, dedicated PLAN-NO-FILE-STRUCTURE/INTERFACES/TASKS tests, plugin.json key-order consistency, AC5 one-line lint delta in `references/spec-construction.md` (plan-mandated CR-005).
