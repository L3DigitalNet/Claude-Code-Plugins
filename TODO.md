...
- [x] **Execute the spec-pipeline implementation plan** — `docs/superpowers/plans/2026-07-01-spec-pipeline-plugin.md` — done 2026-07-02: 14/14 tasks, per-task reviews, final whole-branch review READY TO MERGE (fix commit `4842a7f`), 101/101 tests, all gates green.
- [ ] **spec-pipeline follow-ups** — live smoke test in a fresh session (needs plugin install + cache sync), `/release-pipeline:release` 0.1.0, decide deprecation of `author-master-spec` + `autonomous-phase-execution` in `agent-configs`. 0.1.x hygiene backlog from final review: word-boundary phrase scans, `~~~` fence support, dedicated PLAN-NO-FILE-STRUCTURE/INTERFACES/TASKS tests, plugin.json key-order consistency, AC5 one-line lint delta in `references/spec-construction.md` (plan-mandated CR-005).
